...
> an important insert here:
> the `setup_repository.sh` doesn't automatically push the locally created repo to its github hosted remote - this must be done **manually** , which is kind of an unintended consequential here - so, `cd` into the locally create repo , then `git status` etc , commit, as said, *manually*.
> Even if this wasn't the intended behavior (we wanted to push using `gh`but received "fatal: invalid refspec 'refs/heads/'
failed to run git: exit status 128" errors)  - it makes perfectly sense here. 
